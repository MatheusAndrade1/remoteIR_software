...
def receiveSerial(ser, item):
    ...
    ser.flushInput()
    #time.sleep(.1)
    bytes_ = ser.read(3).hex() # Receive until it finds '\n' by default
    print(bytes_)
    text = str(bytes_).upper()
    print(text)
    output = "0xA1 0xF1 0x"
    for i in range(6):
        if i % 2 == 0 and i!=0:
            ...
            try:
                if ser.isOpen():
                    ...
                    if event == 'DISCONNECT':
                        ...
                        dictionaryToSave['hexCodes']['VOL_UP'] = receiveSerial(ser, 'VOL_UP')
                    elif event == 'VOLDOWN':
                        print('Pressed button VOLUME DOWN')
                        dictionaryToSave['hexCodes']['VOL_DOWN'] = receiveSerial(ser, 'VOL_DOWN')
                    elif event == 'DEL':
            ...
            try:
                if ser.isOpen():
                    ...
                    if event == 'DISCONNECT':
                        ...
                        print('Pressed button VOLUME UP')
                        codeToSend = dictionary['hexCodes']['VOL_UP'].split(" ")
                        ser.write(serial.to_bytes([int(codeToSend[0], base=16), int(codeToSend[1], base=16), int(codeToSend[2], base=16), int(codeToSend[3], base=16), int(codeToSend[4], base=16)]))
                    elif event == 'VOLDOWN':
                        print('Pressed button VOLUME DOWN')
                        codeToSend = dictionary['hexCodes']['VOL_DOWN'].split(" ")
                        ser.write(serial.to_bytes([int(codeToSend[0], base=16), int(codeToSend[1], base=16), int(codeToSend[2], base=16), int(codeToSend[3], base=16), int(codeToSend[4], base=16)]))
                    elif event == 'DEL':
                        print('Pressed button DEL')
